...
<style>
/* ============================================
   EXECUTIVE PALETTE OVERRIDE
   흑연, 아이보리, 샴페인, 스틸 그레이 중심의 더 절제된 프리미엄 톤
   ============================================ */
:root {
    --bg: #060607;
    --bg-soft: #0e0e10;
    --surface: rgba(18, 18, 20, 0.84);
    --surface-2: rgba(28, 27, 29, 0.94);
    --surface-3: rgba(244, 238, 225, 0.055);
    --text: #eee7dc;
    --text-bright: #fffaf0;
    --text-dim: #b4ada2;
    --text-muted: #7d776f;
    --line: rgba(219, 205, 176, 0.16);
    --line-strong: rgba(219, 205, 176, 0.34);
    --gold: #c7ad74;
    --gold-light: #ead9a8;
    --gold-dark: #8d7746;
    --accent: #a9b2bd;
    ...
    --warning: #d6b978;
    --danger: #c97e73;
    --card: rgba(255, 255, 255, 0.046);
    --card2: rgba(255, 255, 255, 0.072);
    --dark: #0b0b0d;
...
.stApp {
    background:
        radial-gradient(circle at 14% -10%, rgba(234, 217, 168, 0.13), transparent 31%),
        radial-gradient(circle at 88% 2%, rgba(169, 178, 189, 0.10), transparent 28%),
        linear-gradient(rgba(255,255,255,0.012) 1px, transparent 1px),
        linear-gradient(90deg, rgba(255,255,255,0.010) 1px, transparent 1px),
        linear-gradient(180deg, #080809 0%, #050506 48%, #0b0b0d 100%) !important;
    background-size: auto, auto, 72px 72px, 72px 72px, auto;
}

[data-testid="stSidebar"] {
    background: linear-gradient(180deg, rgba(18,18,20,0.98) 0%, rgba(9,9,11,0.98) 100%) !important;
}
...
.quick-start-card {
    background:
        linear-gradient(135deg, rgba(234,217,168,0.095), rgba(169,178,189,0.040) 48%, rgba(255,255,255,0.030)),
        rgba(18,18,20,0.84) !important;
...
.interview-hero::before {
    background:
        linear-gradient(120deg, rgba(255,255,255,0.085), transparent 28%),
        radial-gradient(circle at 90% 12%, rgba(234,217,168,0.13), transparent 30%) !important;
}
...
.setup-number {
    color: var(--gold-light) !important;
    border-color: rgba(234,217,168,0.30) !important;
    background: rgba(234,217,168,0.075) !important;
}

.nav-item.active,
.stTabs [aria-selected="true"] {
    background: linear-gradient(135deg, rgba(234,217,168,0.14), rgba(169,178,189,0.08)) !important;
    border-color: rgba(234,217,168,0.28) !important;
}

.section-title-box {
    background:
        linear-gradient(90deg, rgba(234,217,168,0.075), transparent 42%),
...
.login-card {
    background:
        linear-gradient(145deg, rgba(234,217,168,0.085), rgba(169,178,189,0.035)),
        rgba(14,14,16,0.88) !important;
}
...
.stFormSubmitButton > button:hover,
.stLinkButton a:hover {
    border-color: rgba(234,217,168,0.46) !important;
...
[data-testid="baseButton-primary"],
.stDownloadButton > button {
    background: linear-gradient(135deg, #ead9a8 0%, #c7ad74 52%, #8d7746 100%) !important;
    color: #09090a !important;
    -webkit-text-fill-color: #09090a !important;
    border-color: rgba(234,217,168,0.58) !important;
    box-shadow: 0 14px 34px rgba(199,173,116,0.18) !important;
}

.stInfo > div {
    background: rgba(169,178,189,0.11) !important;
    border-color: rgba(169,178,189,0.27) !important;
}
...
if not st.session_state['authenticated']:
    ...
    <div class="login-card">
        ...
        <div class="login-subtitle">Editorial E-Book Studio</div>
    </div>
    """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if "Haiku" in selected_model:
        st.info("💰 가장 저렴하고 빠름.\n📌 고품질 작업은 Sonnet으로 보정되고, 빠른 본문 모드는 Haiku로 속도를 우선합니다.")
    ...
    body_mode_options = {
        "빠른 초안 (챕터 단위 · 추천)": "fast",
        "균형 모드 (챕터 단위)": "balanced",
        "고품질 모드 (소제목별)": "quality",
    }
    if 'body_generation_mode' not in st.session_state:
        st.session_state['body_generation_mode'] = "fast"
    _mode_values = list(body_mode_options.values())
    _mode_labels = list(body_mode_options.keys())
    _mode_index = _mode_values.index(st.session_state.get('body_generation_mode', 'fast')) if st.session_state.get('body_generation_mode', 'fast') in _mode_values else 0
    selected_body_mode = st.selectbox(
        "본문 생성 모드",
        options=_mode_labels,
        index=_mode_index,
        label_visibility="collapsed",
        key="body_generation_mode_label",
    )
    st.session_state['body_generation_mode'] = body_mode_options[selected_body_mode]
    if st.session_state['body_generation_mode'] == "fast":
        st.caption("⚡ 챕터별로 한 번에 생성해 호출 횟수를 크게 줄입니다.")
    elif st.session_state['body_generation_mode'] == "balanced":
        st.caption("⚖️ 속도와 품질을 같이 잡는 모드입니다.")
    else:
        st.caption("💎 가장 느리지만 소제목마다 깊게 씁니다.")
...
def ask_ai(prompt, temp=0.7, ensure_quality=False, max_tokens=8000, model_override=None):
    """Claude API 호출
    ...
    elif ensure_quality and 'haiku' in user_model.lower():
...
# ==========================================
COVER_TEMPLATES = {
    "bestseller_purple": "블랙 에디토리얼 — 대형 타이포 + 퍼플 프레임",
...
def build_cover_svg(template, title, subtitle, author):
    ...
    if template == "bestseller_purple":
        ...
        lower_center_x = 1100 if photo_uri else 800
        lower_main = _wrap_svg_lines(subtitle or "읽히는 문장으로 정리한 한 권", max_chars=18, max_lines=2)
        bg = """
        <defs>
        ...
        kicker = (
            ...
            f'<text x="1295" y="650" text-anchor="middle" font-family="{sans}" font-size="35" '
            f'font-weight="900" fill="#aaa5a0" letter-spacing="5">CASHMAKER</text>'
            f'<text x="1295" y="710" text-anchor="middle" font-family="{sans}" font-size="30" '
            f'font-weight="800" fill="#aaa5a0" letter-spacing="4">EDITORIAL</text>'
        )
        title_block = (
        ...
        sub_block = (
            f'{photo_block}'
            f'<text x="{lower_center_x}" y="1810" text-anchor="middle" font-family="{serif}" '
            f'font-size="62" font-weight="800" fill="#161616" letter-spacing="-1">'
            f'{_svg_tspans(lower_main, lower_center_x, 74)}</text>'
            f'<path d="M{lower_center_x - 250} 2020 H{lower_center_x + 250}" stroke="#6a21b8" stroke-width="8"/>'
            f'<text x="{lower_center_x}" y="2120" text-anchor="middle" font-family="{sans}" '
            f'font-size="36" font-weight="850" fill="#2a2a2a" letter-spacing="2">{author_esc} 지음</text>'
            f'<text x="{lower_center_x}" y="2202" text-anchor="middle" font-family="{sans}" '
            f'font-size="24" font-weight="800" fill="#6a21b8" letter-spacing="5">CASHMAKER BOOKS</text>'
        )
        author_block = (
            f'<rect x="72" y="2236" width="610" height="146" fill="#050505" opacity="{0.94 if photo_uri else 0}"/>'
            f'<text x="376" y="2295" text-anchor="middle" font-family="{sans}" font-size="32" '
            f'font-weight="900" fill="#ffffff" letter-spacing="2">{author_esc if photo_uri else ""}</text>'
        )
        ...
        sub_block = (
            ...
            f'<text x="448" y="2190" text-anchor="middle" font-family="{sans}" font-size="28" font-weight="800" fill="#5b4ca1">소장본 에디션</text>'
            ...
            f'<text x="800" y="2190" text-anchor="middle" font-family="{sans}" font-size="28" font-weight="800" fill="#5b4ca1">여백 조판</text>'
...
def create_ebook_docx(title, subtitle, author, chapters_data, outline, interview_data=None):
    ...
    try:
        ...
        def full_page_cover(title, subtitle, author, brand):
            """A5 한 페이지 안에서 절대 갈라지지 않는 블랙 에디토리얼 표지."""
            _apply_cover_section(doc.sections[-1])
            tbl = doc.add_table(rows=1, cols=1)
            tbl.alignment = WD_TABLE_ALIGNMENT.CENTER

            cell = tbl.rows[0].cells[0]
            tbl.rows[0].height = Cm(18.85)
            tbl.rows[0].height_rule = 2  # EXACT. 한 셀 안에 모든 표지를 넣어 페이지 분리 방지
            cell.width = Cm(13.20)
            va = OxmlElement('w:vAlign'); va.set(qn('w:val'), 'center')
            cell._tc.get_or_add_tcPr().append(va)
            _no_cell_borders(cell)
            _cell_shade(cell, '050505')

            tcMar = OxmlElement('w:tcMar')
            for mn, mv in [('top', '520'), ('left', '560'), ('bottom', '420'), ('right', '560')]:
            ...
            CREAM = (252, 249, 241)
            DIM = (174, 169, 160)
            CHAMPAGNE = (205, 182, 126)

            # 위쪽 얇은 포인트 라인. 텍스트가 아닌 조판 장식만 남긴다.
            top_line = cell.paragraphs[0]
            top_line.alignment = WD_ALIGN_PARAGRAPH.RIGHT
            top_line.paragraph_format.space_before = Pt(0)
            top_line.paragraph_format.space_after = Pt(32)
            set_font(top_line.add_run("━━━━━━━━━━━━"), 9, serif=False, bold=True, color=ACCENT)
            ...
            if subtitle:
                ...
                sp.paragraph_format.line_spacing = 1.28
                set_font(sp.add_run(subtitle), 15.0, serif=True, bold=True, color=CREAM)
            else:
                _cell_para(cell, "읽히는 문장으로 정리한 한 권", 15.0, serif=True, bold=True,
                           color=CREAM, space_after=28, first=False, align=WD_ALIGN_PARAGRAPH.LEFT)

            display_title = re.sub(r'\s*[-–—]\s*', ' ', title or "")
            tlines = _bestseller_title_lines(display_title)
            ...
            if longest <= 4.2:
                ...
                cover_title_size = 27
            if len(tlines) >= 4:
                cover_title_size = min(cover_title_size, 26)
            for line in tlines:
                _cell_para(cell, line, cover_title_size, serif=True, bold=True, color=CREAM,
                           ls=1.02, space_after=0, first=False, align=WD_ALIGN_PARAGRAPH.LEFT)

            mid_line = cell.add_paragraph()
            mid_line.alignment = WD_ALIGN_PARAGRAPH.LEFT
            mid_line.paragraph_format.space_before = Pt(34)
            mid_line.paragraph_format.space_after = Pt(34)
            set_font(mid_line.add_run("━━━━━━"), 11, serif=False, bold=True, color=ACCENT)

            copy = cell.add_paragraph()
            copy.alignment = WD_ALIGN_PARAGRAPH.LEFT
            copy.paragraph_format.space_before = Pt(0)
            copy.paragraph_format.space_after = Pt(42)
            copy.paragraph_format.line_spacing = 1.38
            set_font(copy.add_run("독자가 끝까지 넘기게 만드는 구성과 문장"), 11.2,
                     serif=True, bold=False, color=DIM)
            ...
            set_font(author_p.add_run(f"{author or '저자'} 지음"), 12.4,
            ...
            set_font(brand_p.add_run(brand or "CASHMAKER"), 8.2,
                     serif=False, bold=True, color=CHAMPAGNE, track=1.2)
...
# ==========================================
METHOD_SUFFIXES = ("공식", "법칙", "구조", "전략", "원칙")
METHOD_SUFFIX_LABEL = " / ".join(METHOD_SUFFIXES)


def _valid_method_name_text(name):
    suffix_re = "(" + "|".join(METHOD_SUFFIXES) + ")"
    return bool(name) and re.fullmatch(r"^[A-Z]{2,4}\s" + suffix_re + r"$", (name or "").strip()) is not None
...
def _fallback_method_name(topic):
    ...
    suffix = random.choice(METHOD_SUFFIXES)
...
def get_or_create_method_name(topic, interview_data=None, force_new=False):
    ...
    """
    if not force_new and st.session_state.get('method_name') and _valid_method_name_text(st.session_state.get('method_name')):
...
{{"acronym": "영문 대문자 2~4자", "expansion": "약자 풀이(영문, 하이픈 연결)", "suffix": "한글 접미사", "method_name": "약자 + 공백 + 접미사"}}"""
    ...
    def _valid(n):
        return _valid_method_name_text(n) and n.strip() not in used
...
OUTLINE_FORBIDDEN_TERMS = (
    "루틴", "파이프라인", "엔진", "톱니바퀴", "마법", "황금", "보물", "나침반",
    "연금술", "완벽가이드", "완벽 가이드", "핵심정리", "첫걸음", "첫 설계", "첫 셋업",
    "효과적인", "성공적인", "다양한", "비법", "노하우", "졸업", "마지막 한 수",
    "다른 차원", "결정적 시그널", "진짜 게임", "흔적도 없다", "사건이 아니다",
    "회로가 박혔다", "새로 깔린다", "유전자", "자의식", "추월차선",
)

OUTLINE_BAD_PATTERNS = (
    r"[A-Z]{2,4}\s*(루틴|사이클|파이프라인|엔진)",
    r"[가-힣A-Za-z0-9]+\s*루틴",
    r"(시스템|구조|공식|전략|원칙)(이|가)?\s*(켜|꺼|돌아|굴러|살아|숨쉬|자라|멈춘|작동하)",
    r"(계좌|통장|수익|복리|재투자|배당|멘탈|습관)(이|가|을|를)?\s*(켜|꺼|돌리|굴리|자라|살아|숨쉬|꽂|심)",
    r"(하는|만드는|찾는|배우는|알아보는)\s*(법|방법|가이드|노하우|비법)$",
    r"(의\s*중요성|의\s*이해|의\s*모든\s*것)$",
    r"^\s*(발굴|폭로|전환|설계|실전|수익화|다음\s*단계)\s*$",
)


def _outline_line_issue(text):
    line = re.sub(r'^[\s\-•·*#>]+', '', str(text or "")).strip()
    line = re.sub(r'^(PART|파트|Chapter|챕터)\s*\d+[\.\)]?\s*', '', line, flags=re.IGNORECASE).strip()
    if not line:
        return None

    for term in OUTLINE_FORBIDDEN_TERMS:
        if term in line:
            return f"금지 표현 '{term}'"

    for pat in OUTLINE_BAD_PATTERNS:
        if re.search(pat, line):
            return "어색한 조합 또는 설명식 표현"

    # 자극형 목차라도 한국어 문장으로 읽혀야 하므로 조사/서술 연결이 무너지는 조합을 보수적으로 거른다.
    awkward_chunks = ("배당 사이", "수익 사이", "계좌가 자란", "재투자를 켠", "재투자를 끈", "수익을 켠", "수익을 끈")
    for chunk in awkward_chunks:
        if chunk in line:
            return f"어색한 어법 '{chunk}'"

    return None


def outline_quality_issue(chapters, subtopics):
    for ch in chapters or []:
        issue = _outline_line_issue(ch)
        if issue:
            return f"챕터 '{ch}'에 {issue}"
        for sub in (subtopics or {}).get(ch, []):
            issue = _outline_line_issue(sub)
            if issue:
                return f"소제목 '{sub}'에 {issue}"
    return None


def clean_generated_subtopic(text):
    cleaned = str(text or "").strip().strip('"').strip("'").strip('-').strip()
    return None if _outline_line_issue(cleaned) else cleaned


def parse_outline_text(outline_result):
    chapters = []
    subtopics = {}
    current_ch = None

    for orig_line in str(outline_result or "").split('\n'):
        ...
        clean_line = detect_line.replace('**', '').replace('*', '').replace('#', '').strip()

        is_chapter = False
        ch_name = None
        m = re.match(r'^(?:PART|Part|part)\s*(\d+)[\.\)]?\s*(.+)', clean_line)
        if m:
            is_chapter = True
            ch_name = f"PART {m.group(1)}. {m.group(2).strip()}"
        elif re.match(r'^파트\s*\d+[\.\)]?\s*.+', clean_line):
            is_chapter = True
            ch_name = clean_line
        elif re.match(r'^(?:Chapter|챕터)\s*\d+[\.\)]?\s*.+', clean_line, re.IGNORECASE):
            is_chapter = True
            ch_name = clean_line
        elif re.match(r'^\d+[\.\)]\s*.+', clean_line) and not clean_line.startswith('-'):
            is_chapter = True
            ch_name = f"PART {len(chapters) + 1}. {re.sub(r'^\\d+[\\.]\\)?\\s*', '', clean_line).strip()}"

        if is_chapter and ch_name:
            current_ch = ch_name.replace('**', '').replace('*', '').replace('#', '').strip()
            if current_ch and current_ch not in chapters:
        ...
        if current_ch:
            ...
            if re.match(r'^\s*[\-•·*]\s*', line):
                ...
                st_name = line.lstrip('- •·*→▶0123456789.):\t ').strip()
            ...
            if st_name:
                ...
                st_name = re.sub(r'^\d+[\.\):]\s*', '', st_name).strip()
                if len(st_name) > 3 and len(subtopics[current_ch]) < 4:
...
[이번 버전의 추가 기준 — 정보 제목이 아니라 사건 제목처럼]
- 각 줄은 "무엇을 배운다"가 아니라 "무슨 일이 벌어진다"로 보여야 한다.
- 독자가 줄을 읽자마자 "왜 그렇게 됐지?" 또는 "나는 어디에 해당하지?"를 물어야 한다.
- 돈·투자·부업 주제는 금액, 기간, 계좌 장면, 입금일, 손실 회피를 더 노골적으로 써라.
- 단, 허위 수익 보장처럼 들리면 안 된다. "조건", "차이", "갈림길", "실패 패턴"으로 자극을 만든다.
- "만드는 법", "전략", "파이프라인", "가이드"처럼 이미 답을 알려주는 단어로 끝내지 말고, 결과와 의문만 남겨라.

[자청·프드프식 구매형 목차 원리 — 단어 모방 금지, 구매 심리만 반영]
- 독자의 자기기만을 찌르는 한 줄이 PART마다 있어야 한다.
- "이걸 모르면 손해 본다"는 감각이 제목만 봐도 생겨야 한다.
- 결과는 보여주되 원인과 순서는 숨겨서 본문을 열어보게 만든다.
- 강한 문장이어도 사람을 모욕하지 말고, 행동·착각·선택을 겨냥한다.
- 특정 작가 고유어와 플랫폼 이름을 출력하지 말고, 판매되는 목차의 구조만 가져온다.
- "OOO 루틴", "OOO 파이프라인", "OOO 엔진", "OOO 사이클" 같은 AI식 이름 조합은 단 한 줄도 쓰지 않는다.
...
AI식 접미사: 루틴 / 파이프라인 / 엔진 / 사이클 / 로드맵 / 시스템 첫 설계
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
        ...
        for _outline_attempt in range(2):
            outline_result = ask_ai(outline_prompt, 0.9, ensure_quality=True)
            ...
            # 챕터가 충분히 파싱됐어도 어색한 표현이 있으면 자동 재생성
            quality_issue = outline_quality_issue(chapters, subtopics)
            if quality_issue:
                chapters = []
                subtopics = {}
                continue
        ...
        if not st.session_state.get('outline'):
            ...
            default_chapters = [
                "PART 1. 10명 중 8명이 첫 달에 멈추는 이유",
                "PART 2. 열심히 할수록 결과가 늦어지는 함정",
                "PART 3. 첫 변화가 찍히는 결정적 순간",
                "PART 4. 오래 가는 사람들의 조용한 차이",
                "PART 5. 1년 뒤 완전히 다른 선택"
            ]
            default_subtopics = {
                default_chapters[0]: [f"{topic}을 시작한 사람들이 가장 먼저 착각하는 것", "첫 달에 포기한 사람들의 공통된 장면", "아직 돈을 잃지 않았는데 이미 늦어진 신호", "결국 모든 답은 한 가지 선택으로 좁혀진다"],
                default_chapters[1]: ["더 많이 공부한 사람이 더 늦게 움직이는 이유", "계획표가 촘촘할수록 무너지는 지점", "상위 10%가 처음부터 버린 습관", "누구나 지나치지만 결과를 가르는 작은 차이"],
                default_chapters[2]: ["처음으로 숫자가 달라지는 날의 전조", "남들은 못 보는 첫 번째 기준", "한 번만 바꿨는데 흐름이 달라진 선택", "여기서부터 결과가 쌓이기 시작한다"],
                default_chapters[3]: ["3개월 뒤에도 남아 있는 사람들의 기준", "흔들리는 날에도 계속되는 구조", "성과를 망치는 확인 습관 하나", "조용히 앞서가는 사람들의 반복 방식"],
                default_chapters[4]: ["1년 뒤 같은 자리로 돌아가는 사람들의 패턴", "성과를 지킨 사람들이 다시 하지 않는 일", "다음 단계로 넘어가기 전 확인할 숫자", "결국 오래 남는 사람에게만 보이는 것"]
            }
...
⚠️ 어법 검사 필수: 소리내어 읽었을 때 어색하면 버리고 다시 써라.
⚠️ "OOO 루틴", "OOO 파이프라인", "OOO 엔진", "OOO 사이클" 같은 AI식 이름 조합 절대 금지.

❌ 금지:
- 자청, 역행자, 유전자, 원시인, 추월차선 등 특정 작가 고유 표현
- 설명서 표현: "~의 이해", "~하는 방법"
- 유치한 비유: 나침반, 열쇠, 마법, 톱니바퀴, 파이프라인
- AI식 접미사: 루틴, 엔진, 사이클, 로드맵
- AI 어휘: 효과적인, 다양한, ~를 통해
- 기존 소제목과 비슷한 패턴
...
소제목 하나만 (15~30자, 기호 없이):"""
    ...
    if result:
        return clean_generated_subtopic(result)
    return None
...
소제목 정확히 4개만 출력 (줄바꿈으로 구분, 기호/번호 없이, 각 줄 15~30자):"""
    ...
    if result:
        ...
        for line in result.strip().split('\n'):
            cleaned = clean_generated_subtopic(line)
...
def _body_mode_settings():
    mode = st.session_state.get('body_generation_mode', 'fast')
    if mode == 'quality':
        return {
            "mode": "quality",
            "batch": False,
    ...
    return {
        ...
        "ensure_quality": False,
        "temperature": 0.62,
        "single_max_tokens": 3600,
        "batch_max_tokens": 6500,
    }


def _parse_chapter_batch_body(result, subtopics):
    contents = {}
    key_points = []
    text = str(result or "").replace("\r\n", "\n")

    key_match = re.search(r"\n?###KEYPOINTS\s*\n(?P<body>.*)$", text, flags=re.S | re.I)
    if key_match:
        for line in key_match.group("body").split("\n"):
    ...
    def _match_title(title):
        ...
        for sub in subtopics:
            if clean_title == sub:
                return sub
        for sub in subtopics:
            if clean_title in sub or sub in clean_title:
                return sub
        return None

    for title, body in blocks:
        matched = _match_title(title)
        if not matched:
            continue
        body = re.sub(r"\n?###END\s*$", "", body.strip(), flags=re.I)
        body = clean_content(body, subtopic=matched)
        if body:
            contents[matched] = body

    return contents, key_points[:4]


def generate_body_by_chapter_batch(interview_data, progress_placeholder, settings):
    """빠른/균형 모드: 챕터별 한 번 호출로 여러 소제목 본문을 생성."""
    topic = interview_data.get('topic', '')
    book_concept = st.session_state.get('book_concept', '')
    ...
    total_chapters = len([ch for ch in outline if chapters.get(ch, {}).get('subtopics')])
    completed = 0
    previous_context = []
    ...
    for ch in outline:
        ...
        progress_placeholder.info(
            f"⚡ 챕터 단위 본문 생성 중... ({completed}/{total_chapters}) - {ch[:24]}..."
        )

        subtopic_list = "\n".join(f"{i+1}. {s}" for i, s in enumerate(subtopics))
        previous_text = "\n".join(previous_context[-6:]) if previous_context else "없음"
        mode_name = "빠른 초안" if settings["mode"] == "fast" else "균형 모드"

        prompt = f"""당신은 한국 자기계발 전자책 전문 작가입니다.
아래 챕터의 모든 소제목 본문을 한 번에 작성하세요. 속도 최적화 모드이므로 군더더기 없이 바로 쓸 수 있는 초안을 만듭니다.
...
작성 규칙
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- 각 소제목마다 본문 {settings['target']}.
- 본문 첫 줄에 소제목을 다시 쓰지 말 것.
- 첫 문장은 사건, 숫자, 대사, 반전 중 하나로 시작.
- 독자 직접 호칭 금지: 여러분, 당신, 독자님 금지.
- 특정 작가 고유어 금지: 자청, 역행자, 자의식 해체, 유전자 역행, 추월차선.
- AI식 표현 금지: 효과적인, 다양한, ~를 통해, 결론적으로, 파이프라인, 엔진, 루틴.
- 표는 꼭 필요할 때만 넣고, 빠른 모드에서는 가급적 문단 중심으로 작성.
- 각 소제목은 서로 다른 장면과 통찰을 가져야 하며 같은 말을 반복하지 말 것.
- 짧은 문장과 긴 문장을 섞어 리듬 있게 쓸 것.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
출력 형식
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
아래 형식을 정확히 지키세요. 마크다운 제목, 번호, 설명문은 추가하지 마세요.

###SUBTOPIC: {subtopics[0]}
본문

###SUBTOPIC: {subtopics[1] if len(subtopics) > 1 else subtopics[0]}
본문
...
- 이 챕터의 핵심 한 문장"""
        ...
        raw = ask_ai(
            ...
            ensure_quality=settings["ensure_quality"],
            model_override=settings["model_override"],
            max_tokens=settings.get("batch_max_tokens", 8000),
        )
        contents, key_points = _parse_chapter_batch_body(raw, subtopics)

        # 일부 소제목이 누락되면 해당 소제목만 개별 생성해서 안정성을 보완한다.
        for sub in subtopics:
            if sub not in contents:
                fallback = generate_content_premium(sub, ch, [], [], topic, st.session_state.get('target_persona', ''))
                if fallback:
                    contents[sub] = fallback

        for sub in subtopics:
            if contents.get(sub):
                ch_data.setdefault('subtopic_data', {}).setdefault(sub, {'questions': [], 'answers': [], 'content': ''})
                ch_data['subtopic_data'][sub]['content'] = contents[sub]

        if key_points:
            ch_data['key_points'] = key_points
        else:
            ch_data['key_points'] = [
                re.sub(r"\s+", " ", contents.get(s, "").strip())[:45]
                for s in subtopics if contents.get(s)
            ][:3]

        previous_context.append(f"- {ch}: " + " / ".join(ch_data.get('key_points', [])[:3]))
...
def generate_body_from_outline(interview_data, progress_placeholder):
    ...
    try:
        ...
        settings = _body_mode_settings()
        if settings.get("batch"):
            return generate_body_by_chapter_batch(interview_data, progress_placeholder, settings)
...
def auto_generate_all(topic, progress_placeholder):
    ...
    try:
        ...
        if st.session_state.get('outline') and st.session_state.get('chapters'):
            interview_data = dict(st.session_state.get('interview_data') or st.session_state.get('temp_interview') or {})
            interview_data.setdefault('topic', topic)
            interview_data.setdefault('author_name', st.session_state.get('author_name', '') or '저자')
            if not generate_body_from_outline(interview_data, progress_placeholder):
                return False
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
    last_result = ""
    for _ in range(2):
        result = ask_ai(prompt, 0.9, ensure_quality=True)
        if not result:
            continue
        last_result = result
        parsed_chapters, parsed_subtopics = parse_outline_text(result)
        if parsed_chapters and not outline_quality_issue(parsed_chapters, parsed_subtopics):
            return result
    return last_result
...
def generate_content_premium(subtopic, chapter, questions, answers, topic, persona):
    ...
    settings = _body_mode_settings()
    target_length = settings.get("target", "2400~3000자")
    model_override = settings.get("model_override")
    ensure_quality = settings.get("ensure_quality", True)
    temperature = settings.get("temperature", 0.75)
    max_tokens = settings.get("single_max_tokens", 8000)
    mode_label = {
        "fast": "빠른 초안",
        "balanced": "균형 모드",
        "quality": "고품질 모드",
    }.get(settings.get("mode"), "고품질 모드")
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📏 분량: {target_length} (반복 없이 전문성 있는 밀도로)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
if header_video_b64:
    ...
    <style>
        ...
        .hero .veil {{
            ...
            background:
                radial-gradient(circle at 18% 22%, rgba(234,217,168,0.16), transparent 32%),
                radial-gradient(circle at 76% 10%, rgba(169,178,189,0.12), transparent 30%),
        ...
        .hero .status-dot {{
            width:9px; height:9px; border-radius:99px; background:#c7ad74;
            box-shadow:0 0 24px rgba(199,173,116,.80);
        }}
        .eyebrow {{
            color:#ead9a8; font-size:12px; letter-spacing:.2em;
        ...
        .hero .hud-card {{
            border:1px solid rgba(234,217,168,.16);
            background:rgba(7,7,8,.50);
        ...
        .hero .bottom-line {{
            ...
            background:linear-gradient(90deg,#c7ad74,rgba(199,173,116,0));
        }}
    ...
    <body>
        <div class="hero">
            ...
            <div class="center">
                <div class="panel">
                    <div class="eyebrow-row"><span class="status-dot"></span><div class="eyebrow">Editorial Production Suite</div></div>
            ...
            <div class="hud">
                <div class="hud-card"><div class="hud-label">Work</div><div class="hud-value">Studio</div></div>
                <div class="hud-card"><div class="hud-label">File</div><div class="hud-value">DOCX</div></div>
                <div class="hud-card"><div class="hud-label">Layout</div><div class="hud-value">Book</div></div>
            </div>
        </div>
    </body></html>
    """, height=452)
            ...
            if not st.session_state.get('topic'):
                ...
                with st.spinner("목차 생성 중..."):
                    ...
                    if result:
                        ...
                        quality_issue = outline_quality_issue(chapters, subtopics)
                        if quality_issue:
                            st.error(f"목차 어법 검사에서 걸렸습니다: {quality_issue}. 다시 생성해주세요.")
                        elif chapters:
                            st.session_state['outline'] = chapters
                            st.session_state['chapters'] = {}
                            for ch in chapters:
                    ...
                    if has_ans:
                        if st.button("본문 생성", key=f"gen_content_{st_key}", use_container_width=True, type="primary"):
                            if not get_api_key():
                                ...
                                body_mode = st.session_state.get('body_generation_mode', 'fast')
                                spinner_label = "빠른 본문 작성 중..." if body_mode == "fast" else "본문 작성 중..."
...
{{
    ...
    with col2:
        ...
        st.caption(
            "블랙 에디토리얼 스타일은 cover_photo.jpg/png 또는 author_photo.jpg/png를 "
