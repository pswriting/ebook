...
자, 이제 시작합니다."""
        ...
        for para_text in prologue_text.split('\n\n'):
            if para_text.strip():
                ...
                para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY  # 양쪽 맞춤
...
if not st.session_state.get('interview_completed', False):
    ...
    if step == 1:
        ...
        st.text_input("저자명 (필명 가능)", value=st.session_state['temp_interview'].get('author_name', ''), placeholder="예: 김성장, 머니메이커 등", key="step1_author")
        st.text_input("당신의 전문 분야는?", value=st.session_state['temp_interview'].get('field', ''), placeholder="예: 주식투자, 블로그 수익화, 다이어트, 영어회화 등", key="step1_field")
        ...
        st.selectbox("이 분야 경험은?", exp_options, index=exp_index, key="step1_exp")

        col1, col2 = st.columns([1, 1])
        with col2:
            if st.button("다음 →", key="interview_next_1", use_container_width=True, type="primary"):
                # 세션 상태에서 직접 값 읽기
                author_name = st.session_state.get('step1_author', '').strip()
                field = st.session_state.get('step1_field', '').strip()
                experience = st.session_state.get('step1_exp', '선택하세요')

                if not author_name or not field or experience == "선택하세요":
                    st.error("모든 항목을 입력해주세요")
                else:
                    st.session_state['temp_interview']['author_name'] = author_name
                    st.session_state['temp_interview']['field'] = field
    ...
    # ========== STEP 2: 주제와 노하우 ==========
        ...
        with col1:
            if st.button("← 이전", key="interview_prev_2", use_container_width=True):
                ...
                st.rerun()
        with col2:
            if st.button("다음 →", key="interview_next_2", use_container_width=True, type="primary"):
                if not topic or not core_method:
                    st.error("모든 항목을 입력해주세요")
                else:
                    st.session_state['temp_interview']['topic'] = topic
                    st.session_state['temp_interview']['core_method'] = core_method
    ...
    # ========== STEP 6: 목차 확인 및 본문 생성 ==========
        ...
        if outline:
            ...
            for i, ch in enumerate(outline):
                ...
                with col_ch2:
                    if st.button("🔄", key=f"regen_ch_{i}", help="이 챕터 소제목 모두 재생성"):
                        with st.spinner("재생성 중..."):
                            new_subs = regenerate_chapter_subtopics(ch, i)
                            if new_subs:
                                st.session_state['chapters'][ch]['subtopics'] = new_subs
                                st.session_state['chapters'][ch]['subtopic_data'] = {
                                    s: {'questions': [], 'answers': [], 'content': ''} for s in new_subs
                                }
                                st.rerun()
                ...
                for j, sub in enumerate(subtopics):
                    ...
                    with col2:
                        ...
                        if new_sub != sub and new_sub.strip():
                            ...
                            st.rerun()
                    with col3:
                        if st.button("🔄", key=f"regen_sub_{i}_{j}", help="이 소제목만 재생성"):
                            with st.spinner("..."):
                                new_subtopic = regenerate_single_subtopic(ch, j, subtopics)
                                if new_subtopic:
                                    old_sub = st.session_state['chapters'][ch]['subtopics'][j]
                                    st.session_state['chapters'][ch]['subtopics'][j] = new_subtopic
                                    # subtopic_data 키도 변경
                                    old_data = st.session_state['chapters'][ch]['subtopic_data'].pop(old_sub, {'questions': [], 'answers': [], 'content': ''})
                                    st.session_state['chapters'][ch]['subtopic_data'][new_subtopic] = old_data
                                    st.rerun()

            st.markdown("---")
            st.info("💡 **Tip**: 텍스트를 직접 수정하거나, 🔄 버튼으로 AI가 새로운 아이디어를 제안합니다.")
        ...
        with col1:
            if st.button("← 이전", key="interview_prev_6", use_container_width=True):
                ...
                st.rerun()
        with col2:
            if st.button("🔄 전체 목차 재생성", key="regenerate_outline", use_container_width=True):
                with st.spinner("목차 재생성 중... (약 30초 소요)"):
                    progress_box = st.empty()
                    interview_data = st.session_state.get('temp_interview', {})
                    # topic이 없으면 session_state에서 가져오기
                    if not interview_data.get('topic') and st.session_state.get('topic'):
                        interview_data['topic'] = st.session_state.get('topic')
                    success = generate_outline_only(interview_data, progress_box)
                    if success:
                        st.rerun()
                    else:
                        st.error("목차 재생성에 실패했습니다. API 키를 확인해주세요.")
        with col3:
