...
if header_video_b64:
    ...
    # 영상은 st.markdown이 <video> 태그를 제거하므로 components.html(iframe)로 렌더링
    components.html(f"""
    <style>
    html,body {{ margin:0; padding:0; background:transparent; overflow:hidden; }}
    .v-wrap {{ position:relative; border-radius:16px; overflow:hidden; border:0.5px solid rgba(201,162,75,0.18); height:300px; }}
    .v-wrap video {{ width:100%; height:300px; object-fit:cover; filter:brightness(0.30) saturate(1.1); display:block; }}
    .v-grad {{ position:absolute; inset:0; background:linear-gradient(180deg, rgba(11,11,13,0.40) 0%, rgba(11,11,13,0.72) 100%); }}
    .v-center {{ position:absolute; inset:0; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center; }}
    .v-eyebrow {{ display:flex; align-items:center; justify-content:center; gap:16px; margin-bottom:22px; }}
    .v-eyebrow .we-text {{ color:#E8CF8C; font-size:11px; letter-spacing:0.45em; text-indent:0.45em; font-weight:500; font-family:'Pretendard',sans-serif; }}
    .v-eyebrow .we-line-l {{ width:30px; height:1px; background:linear-gradient(90deg,transparent,#C9A24B); }}
    .v-eyebrow .we-line-r {{ width:30px; height:1px; background:linear-gradient(90deg,#C9A24B,transparent); }}
    .v-title {{ font-family:'S-CoreDream','Pretendard',sans-serif; font-size:60px; font-weight:300; letter-spacing:0.16em; text-indent:0.16em; margin:0;
        background:linear-gradient(180deg,#FFFFFF 0%,#F4ECD4 52%,#D8B45E 130%); -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent;
        filter:drop-shadow(0 2px 14px rgba(201,162,75,0.28)); }}
    .v-divider {{ display:flex; align-items:center; justify-content:center; gap:10px; margin:24px auto 0; }}
    .v-divider .wd-line-l {{ width:42px; height:1px; background:linear-gradient(90deg,transparent,#C9A24B); }}
    .v-divider .wd-line-r {{ width:42px; height:1px; background:linear-gradient(90deg,#C9A24B,transparent); }}
    .v-divider .wd-diamond {{ width:5px; height:5px; background:#C9A24B; transform:rotate(45deg); box-shadow:0 0 8px rgba(201,162,75,0.7); }}
    .v-tagline {{ color:#C9C6BE; font-size:15px; margin-top:18px; font-weight:300; letter-spacing:0.03em; font-family:'Pretendard',sans-serif; }}
    </style>
    <div class="v-wrap">
        ...
        <div class="v-center">
            <div class="v-eyebrow"><span class="we-line-l"></span><span class="we-text">PREMIUM EBOOK MAKER</span><span class="we-line-r"></span></div>
            <h1 class="v-title">WRITEY</h1>
            <div class="v-divider"><span class="wd-line-l"></span><span class="wd-diamond"></span><span class="wd-line-r"></span></div>
            <p class="v-tagline">6개의 질문에 답하면 AI가 목차부터 본문까지 완성합니다</p>
        </div>
    </div>
    <script>
        var _v = document.querySelector('video');
        if (_v) {{ _v.muted = true; var _p = _v.play(); if (_p && _p.catch) _p.catch(function(){{}}); }}
    </script>
    """, height=302)
